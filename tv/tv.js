...
async function getMovieDetails(id) {
  try {
    const response = await fetch(`${URL}/3/tv/${id}`, options);
    if (!response.ok) {
...
async function getMovieCast(id) {
  try {
    const response = await fetch(`${URL}/3/tv/${id}/credits`, options);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
...
async function getRecommendations(id) {
  try {
    const response = await fetch(`${URL}/3/tv/${id}/recommendations`, options);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
...
async function getTv(id) {
  try {
    const response = await fetch(`${URL}/3/tv/${id}/videos`, options);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
...
async function landingPage() {
  ...
  landingPage.style.backgroundImage = `url("${baseImageUrl}${backdrop_path}")`;
  container.innerHTML += `
